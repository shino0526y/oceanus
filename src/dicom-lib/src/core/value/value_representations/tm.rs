...
pub use tm_value::{TmValue, TmValueError};
...
pub struct Tm {
    values: Vec<Option<TmValue>>,
}

impl Tm {
    pub fn values(&self) -> &Vec<Option<TmValue>> {
        &self.values
    }
    ...
    pub fn from_string(str: &str) -> Result<Self, MultiStringValueError> {
        ...
        for (i, str) in strings.iter().enumerate() {
            ...
            let value = TmValue::from_string(str).map_err(|error| {
...
mod tests {
    ...
    fn test_from_string() {
        // 正常系: 空の文字列
        {
            // Arrange
            let input = "";
            let expected = Tm { values: vec![None] };
        ...
        // 正常系: 単一値
        {
            ...
            let input = "103045";
            let expected = Tm {
                values: vec![Some(TmValue::from_string("103045").unwrap())],
            };
        ...
        // 正常系: 複数値
        {
            ...
            let expected = Tm {
                values: vec![
                    Some(TmValue::from_string("103045").unwrap()),
                    Some(TmValue::from_string("120000").unwrap()),
                ],
            };
        ...
        // 正常系: 複数値で空の値を含むケース
        {
            ...
            let input = r"\103045";
            let expected = Tm {
                values: vec![None, Some(TmValue::from_string("103045").unwrap())],
            };
